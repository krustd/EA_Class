...
The central business process is \textbf{Fulfill Customer Order}, which decomposes into four sub-processes:
...
\begin{enumerate}[leftmargin=1.3em, nosep]
  ...
  \item \textbf{Settle Payment} --- Payment is processed (pre-paid or on-collection), and the transaction is recorded.
\end{enumerate}
...
\begin{tikzpicture}[
  ...
  \node[box, below=0.33cm of receive] (prepare) {Prepare Drink};
  \node[box, below=0.33cm of prepare] (deliver) {Deliver or Pick Up};
  \node[box, below=0.33cm of deliver] (settle) {Settle Payment};
  \node[event, below=0.6cm of settle] (end) {Order Fulfilled (End)};

  \draw[arrow] (trigger) -- (receive);
  \draw[arrow] (receive) -- (prepare);
  \draw[arrow] (prepare) -- (deliver);
  \draw[arrow] (deliver) -- (settle);
  \draw[arrow] (settle) -- (end);
\end{tikzpicture}
\end{center}
...
\caption{Application Layer Model for Luckin Coffee --- showing customer-facing application interfaces, application services, the Luckin Digital Platform components, and managed data objects.}
\label{fig:alm}
\end{figure}
...
The Technology Layer describes the physical infrastructure --- devices, networks, platform hosting, system software, and technology artifacts --- on which the application components run. It answers the question: \textit{what hardware, network, and software infrastructure supports the applications?}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth,keepaspectratio]{TLM.png}
\caption{Technology Layer Model for Luckin Coffee --- showing devices, communication networks, the cloud platform, system software, and technology artifacts.}
\label{fig:tlm}
\end{figure}
...
All devices connect through a shared communication layer: \textbf{Internet / 4G / 5G / Wi-Fi}. This reflects the reality that Luckin's architecture must support diverse connectivity --- customers on mobile networks, stores on broadband or Wi-Fi, IoT sensors on local wireless, and delivery riders on 4G/5G.

\subsubsection{Technology Platform and System Software}

The core hosting environment is a \textbf{Cloud Platform}. External traffic reaches the platform through a \textbf{Platform Access Interface} that routes requests from the communication network into the cloud infrastructure. The system software stack running on the platform comprises:
...
\begin{tabularx}{\textwidth}{P{2.2cm}P{2.4cm}P{2.2cm}P{1.6cm}P{3.2cm}P{1.8cm}P{1.6cm}}
\toprule
\textbf{Order\_ID} & \textbf{Customer\_Name} & \textbf{Customer\_Phone} & \textbf{Order\_Date} & \textbf{Store\_Name, City} & \textbf{\{Product\_Name\}} & \textbf{\{Qty\}} \\
\midrule
...
\footnotesize
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{\textwidth}{P{1.5cm}P{2.0cm}P{2.0cm}P{1.8cm}P{2.2cm}P{1.8cm}P{1.2cm}P{1.2cm}}
\toprule
\textbf{OrdID} & \textbf{CustName} & \textbf{CustPhone} & \textbf{OrdDate} & \textbf{StoreName, City} & \textbf{ProdName} & \textbf{Qty} & \textbf{Price} \\
\midrule
...
\toprule
\pk{OrdID} & CustName & CustPhone & OrdDate \\
\midrule
...
\textbf{OrderItems}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{P{1.3cm}P{1.8cm}P{0.8cm}P{1.0cm}}
\toprule
\pk{OrdID}\pkfk & \pk{ProdName} & Qty & Price \\
\midrule
1001 & Latte     & 1 & 15.0 \\
...
\toprule
\pk{OrdID}\pkfk & \pk{ProdID}\pkfk & Qty & Price \\
\midrule
1001 & 101 & 1 & 15.0 \\
1001 & 201 & 2 & 8.0  \\
1002 & 102 & 1 & 12.0 \\
...
\textbf{Note on derived attributes and deliberate denormalization.}
The final schema in Table~19 includes attributes such as \texttt{Subtotal} (in OrderItems), \texttt{Total\_Amount}, \texttt{Discount\_Amount}, and \texttt{Net\_Amount} (in DigitalOrders). In a strict normalization-only context, these are \textbf{derived attributes}: \texttt{Subtotal} = Quantity $\times$ Unit\_Price, and \texttt{Net\_Amount} = Total\_Amount $-$ Discount\_Amount. Strictly speaking, derived attributes violate the non-redundancy principle of 3NF and should be computed on the fly.

However, in a high-throughput retail platform like Luckin, computing these values at query time across millions of orders would impose unacceptable latency. The schema therefore retains these fields as a \textbf{deliberate denormalization} for read performance, transaction logging (the stored value reflects the state \emph{at the time of order}, not the current product price), and auditability. This is an explicit, well-understood trade-off between normalization purity and operational efficiency, common in production OLTP systems. The denormalized fields are protected by triggers that ensure computed values remain consistent with their source columns.
...
\renewcommand{\arraystretch}{1.22}
\begin{longtable}{P{2.6cm}P{2.2cm}P{2.8cm}P{6.2cm}}
...
\section{Business Scenarios Supported by the Refined Schema}
...
\subsection{Scenario 1: Mobile Order and Pickup with Promotions}
...
\begin{itemize}[leftmargin=1.3em]
  \item \textbf{Relations joined}: Customers $\rightarrow$ MembershipAccounts $\rightarrow$ MemberPromotions $\rightarrow$ Promotions $\rightarrow$ Campaigns $\rightarrow$ DigitalOrders $\rightarrow$ OrderItems $\rightarrow$ Products $\rightarrow$ Payments $\rightarrow$ FulfilmentTasks $\rightarrow$ Stores.
  \item \textbf{Business value}: The schema traces the full promotional journey --- which campaign issued the coupon, whether the member used it, which products it applied to, and whether fulfilment was timely. This enables campaign ROI analysis.
\end{itemize}

\subsection{Scenario 2: Store-Level Inventory Replenishment}

When a store's inventory of a product or material falls below its threshold:

\begin{itemize}[leftmargin=1.3em]
  \item \textbf{Relations joined}: Stores $\rightarrow$ Inventory $\rightarrow$ Products; Stores $\rightarrow$ StoreMaterials $\rightarrow$ RawMaterials $\rightarrow$ Suppliers.
  \item \textbf{Business value}: The system can identify low-stock items by store, group replenishment needs by supplier, and trigger automatic purchase orders. Inventory data also feeds demand forecasting models.
\end{itemize}

\subsection{Scenario 3: Customer Feedback and Service Improvement}

When a customer submits a rating and comment after order completion:

\begin{itemize}[leftmargin=1.3em]
  \item \textbf{Relations joined}: Feedback $\rightarrow$ Customers $\rightarrow$ DigitalOrders $\rightarrow$ Stores $\rightarrow$ FulfilmentTasks.
  \item \textbf{Business value}: Low ratings can be correlated with specific stores, order types, fulfilment delays, or product quality issues. This enables data-driven service improvement at the store and process level.
\end{itemize}

\subsection{Scenario 4: Campaign Performance Analysis}

When the marketing team evaluates a campaign's effectiveness:

\begin{itemize}[leftmargin=1.3em]
  \item \textbf{Relations joined}: Campaigns $\rightarrow$ Promotions $\rightarrow$ MemberPromotions $\rightarrow$ MembershipAccounts $\rightarrow$ DigitalOrders $\rightarrow$ OrderItems $\rightarrow$ Products.
  \item \textbf{Business value}: Analysts can compute redemption rates, incremental revenue per campaign, and customer segment responsiveness, enabling data-driven marketing budget allocation.
\end{itemize}

% =================== PART IV: ARCHITECTURE-DATABASE ALIGNMENT ===================
\section{Alignment Between Architecture Layers and Database Schema}

A key deliverable of Seminar 6 is demonstrating how the ArchiMate layered architecture models map to the relational database design. The following table traces key entities from business concept through application data object to database table:

\begin{table}[H]
\centering
\caption{Vertical traceability: business concept $\rightarrow$ application data $\rightarrow$ database table.}
\small
\begin{tabularx}{\textwidth}{P{3.2cm}P{3.5cm}P{3.5cm}Y}
\toprule
\textbf{Business Concept (BLM)} & \textbf{Data Object (ALM)} & \textbf{Database Table} & \textbf{How the Table Realizes the Concept} \\
\midrule
...
This alignment ensures that the database schema is not an isolated technical artifact --- it is the persistent data layer that supports every business process described in the Business Layer Model and every application component described in the Application Layer Model.
